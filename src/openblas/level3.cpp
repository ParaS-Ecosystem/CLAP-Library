...
void OpenBlasBackend::symm(Layout layout, Side side,
                Uplo uplo, int64_t m, int64_t n,
                 ...
                 float *C, int64_t ldc)
                 {
                  ...
                  to_cblas_uplo(uplo), (int) m, (int) n,
                  alpha,  A, (int) lda,
                  B, (int) ldb,  beta,
                  C, (int) ldc);
}
...
void OpenBlasBackend::syrk(Layout layout,Uplo uplo,
                 Transpose transA, int64_t n, int64_t K,
                 const float alpha, const float *A, int64_t lda,
                 const float beta, float *C, int64_t ldc){
              
                    clap_cblas_ssyrk(to_cblas_order(layout),  to_cblas_uplo(uplo),
                 to_cblas_trans(transA), (int) n, (int) K,
                  alpha,  A, (int) lda,
                  beta,  C, (int) ldc);
}
...
void OpenBlasBackend::trmm(Layout layout, Side side,
                Uplo uplo, Transpose transA,
                 Diag diag, int64_t m, int64_t n,
                 const float alpha, const float *A, int64_t lda,
                 float *B, int64_t ldb)
                 {
                 ...
                 to_cblas_diag(diag), (int) m, (int) n,
                  alpha,  A, (int) lda,
                  B, (int) ldb);
}
...
void OpenBlasBackend::trsm(Layout layout, Side side,
          Uplo uplo, Transpose transA,
          Diag diag, int64_t m, int64_t n,
          float alpha, const float* A, int64_t lda,
          float* B, int64_t ldb)
